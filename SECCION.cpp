...
void SECCION::insertarProducto(string nombre, int codigo, int precio, int inventario, int inventarioM){
    PRODUCTO *nuevo = new PRODUCTO(nombre, codigo, precio, inventario, inventarioM);
    if(estaVacio()){
        Cabecera = nuevo;
    }
    else{
...
int SECCION::Imprimir2(PRODUCTO *aux, int y){
    if(aux != NULL){
        y = Imprimir2(aux->getIzq(), y);
        gotoxy(8,y); cout<<aux->getCodigo();
        gotoxy(24,y); cout<<aux->getNombre();
        gotoxy(48,y); cout<<"$"<<aux->getPrecio();
        gotoxy(64,y); cout<<aux->getInventario();
        y++;
        y = Imprimir2(aux->getDer(), y);
        return y++;
    }
}
...
void SECCION::ventaProducto(int codigo, int cantidad){
    PRODUCTO *ant = nullptr;
    PRODUCTO *act = Cabecera;
    while((act != nullptr) && (act->getCodigo() != codigo)){
        ant = act;
        if (act->getCodigo() < codigo){
        	act = act->getDer();
        }
    ...
    if(act != NULL){
        int n = act->getInventario() - cantidad;
        act->setInventario(n);
    }
}

void SECCION::reponerProducto(int codigo, int cantidad){
    PRODUCTO *ant = nullptr;
    PRODUCTO *act = Cabecera;
    while((act != nullptr) && (act->getCodigo() != codigo)){
        ant = act;
...
void SECCION::cambiarPrecio(int codigo){
    PRODUCTO *act = Cabecera;
    while((act != nullptr) && (act->getCodigo() != codigo)){
        if (act->getCodigo() < codigo){
        	act = act->getDer();
    ...
    if(act!= NULL){
        int nuevo;
        gotoxy(8,3); cout<<"Ingrese el nuevo precio para "<<act->getNombre()<<": ";
        cin>>nuevo;
        act->setPrecio(nuevo);
    }
}
